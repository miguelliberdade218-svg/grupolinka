...
# VERSÃO FINAL v2.2 - 09/01/2026 - HOTEL EM TOFO COM COORDENADAS REAIS
# =============================================================================
...
test_endpoint "3. Buscar Hotel por ID" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID" "noauth"
...
# 5. Listar Room Types
test_endpoint "5. Listar Room Types" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/room-types" "noauth"
...
test_endpoint "7. Buscar Disponibilidade" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/availability?startDate=$NEXT_WEEK&endDate=$NEXT_WEEK_PLUS_3&roomTypeId=$TEST_ROOM_TYPE_ID" "noauth"
...
# 9. Listar Promoções
test_endpoint "9. Listar Promoções" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/promotions" "noauth"

# 10. Calcular Preço com Promoção
test_endpoint "10. Calcular Preço com Promoção" "POST" "$HOTEL_API_BASE/$TEST_HOTEL_ID/bookings/calculate-price" "noauth" "{\"roomTypeId\": \"$TEST_ROOM_TYPE_ID\", \"checkIn\": \"$NEXT_WEEK\", \"checkOut\": \"$NEXT_WEEK_PLUS_3\", \"units\": 1, \"promoCode\": \"TOFO2026\"}"
...
# 12. Check-in
test_endpoint "12. Check-in" "POST" "$HOTEL_API_BASE/bookings/$TEST_BOOKING_ID/check-in" "auth" "{}"

sleep 3

# 13. Check-out
test_endpoint "13. Check-out" "POST" "$HOTEL_API_BASE/bookings/$TEST_BOOKING_ID/check-out" "auth" "{}"

# 14. Submeter Review
log_test "14. Submeter Review"
review_data="{
...
# 15. Listar Reviews
test_endpoint "15. Listar Reviews" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/reviews" "noauth"

# 16. Estatísticas Reviews
test_endpoint "16. Estatísticas Reviews" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/reviews/stats" "noauth"

# 17. Votar Review como útil
if [ -n "$TEST_REVIEW_ID" ]; then
    log_test "17. Votar Review como útil"
...
# 19. Limpeza opcional
log_header "LIMPEZA OPCIONAL"
echo -e "${YELLOW}Itens criados:${NC}"
...
if [[ "$cleanup" =~ ^[Ss]$ ]]; then
    ...
    [ -n "$TEST_BOOKING_ID" ] && curl -s -X POST "$HOTEL_API_BASE/bookings/$TEST_BOOKING_ID/cancel" -H "Authorization: Bearer $BEARER_TOKEN" -d '{"reason":"Limpeza de teste"}' || true
...
log_header "TESTE FINALIZADO COM SUCESSO!"
echo -e "${GREEN}Tudo testado: Criação, CRUD, Room Types, Promoção, Reserva, Pagamento, Review${NC}"
echo "Data: $(date)"
echo "=================================================="
