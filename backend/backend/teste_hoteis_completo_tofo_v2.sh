...
# VERSÃO FINAL MELHORADA - 09/01/2026 - HOTEL EM TOFO COM COORDENADAS REAIS
# =============================================================================
...
test_endpoint() {
    ...
    if [[ "$status" =~ ^(200|201|204)$ ]]; then
        ...
        # Continua mesmo com erro (não mata o script)
    fi
...
# 9. Calcular Preço com Promoção
test_endpoint "9. Calcular Preço com Promoção" "POST" "$HOTEL_API_BASE/$TEST_HOTEL_ID/bookings/calculate-price" "noauth" "{\"roomTypeId\": \"$TEST_ROOM_TYPE_ID\", \"checkIn\": \"$NEXT_WEEK\", \"checkOut\": \"$NEXT_WEEK_PLUS_3\", \"units\": 1, \"promoCode\": \"TOFO2026\"}"

# 10. Criar Reserva
log_test "10. Criar Reserva"
booking_data="{
...
# 11. Check-in
test_endpoint "11. Check-in" "POST" "$HOTEL_API_BASE/bookings/$TEST_BOOKING_ID/check-in" "auth" "{}"

sleep 2

# 12. Check-out
test_endpoint "12. Check-out" "POST" "$HOTEL_API_BASE/bookings/$TEST_BOOKING_ID/check-out" "auth" "{}"

# 13. Submeter Review
log_test "13. Submeter Review"
review_data="{
...
# 14. Listar Reviews
test_endpoint "14. Listar Reviews" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/reviews" "noauth"

# 15. Estatísticas Reviews
test_endpoint "15. Estatísticas Reviews" "GET" "$HOTEL_API_BASE/$TEST_HOTEL_ID/reviews/stats" "noauth"
