...
export interface EmailService {
  ...
  sendVerificationApprovedEmail(to: string, name: string, capacity: 'drive' | 'hotel_manager'): Promise<boolean>;
  sendVerificationRejectedEmail(to: string, name: string, capacity: 'drive' | 'hotel_manager', reason?: string): Promise<boolean>;
}
...
export class ResendEmailService implements EmailService {
  ...
  constructor() {
    ...
    this.fromEmail = import.meta.env.VITE_EMAIL_FROM || 'noreply@linka.co.mz';
    ...
    if (this.isEnabled && apiKey) {
      ...
      console.warn('⚠️ Email Service: VITE_RESEND_API_KEY não configurada. Emails serão logados no console.');
    }
  }
